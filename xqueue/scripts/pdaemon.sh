...
BASE_DIR="/var/www/mooc-linux-programming/"
...
#	fi
	...
	# Checking http_ext_lock file existance and task_id == "HTTP_EXT_LOCK"
	# If file exists then isolate system from the external connections 
	if [ -f "$HTTP_EXT_LOCK" ] && [ "$task_id" == "$HTTP_EXT_LOCK_ID" ];
	then
		log "$HTTP_EXT_LOCK found, locking external contections to apache"
	...
	# Running check
        old_pwd=$(pwd)
    	cd "${tests_dir}"
    	log "Calling checker_plug.py: id=${id}, task_id=${task_id}"
    	python3 ./checker_plug.py "${id}" "${task_id}" "${solution}" "${makefile}" > /tmp/checker_out.log 2>&1
    	result=$?
    	log "checker_plug.py exit code: $result"
    	cat /tmp/checker_out.log | while IFS= read -r line; do
        	log "$line"
    	done
